...
class Database:
    ...
    async def add_new_color(self, color: Color):
        if await self.is_color_exists(color.owner, color.name):
            raise HTTPException(status_code=409, detail=f"Color {color.name} already exists")
        
        for color_value in [color.red, color.green, color.blue]:
            if not 0 <= color_value <= 255:
                raise HTTPException(status_code=400, detail="Value error. Color value must be >0 and <255")
